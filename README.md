 <div align="center">

  <!-- 👋 Name Header -->
  <h1 style="background: linear-gradient(90deg,#22D3EE,#6366F1); -webkit-background-clip: text; color: transparent;">
    👋 Hi, I'm <strong>Uday Raut</strong>
  </h1>

  <!-- Typing Animation -->
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=28&duration=3500&pause=1000&color=22D3EE&background=0D111700&center=true&vCenter=true&width=600&lines=Cyber+Security;Penetration+Tester;Ethical+Hacker;Bug+Bounty+Learner" alt="Typing Animation">

  <!-- Profile Badges -->
  <p>
    <img src="https://komarev.com/ghpvc/?username=udayraut128&label=Profile+Views&color=0e75b6&style=flat" alt="Profile Views">
    <img src="https://img.shields.io/badge/Focus-Cybersecurity-red?style=flat-square" alt="Focus">
    <img src="https://img.shields.io/badge/Open%20to%20Work-Yes-success?style=flat-square" alt="Open to Work">
  </p>

</div>

---

## 🎯 About Me
<div align="center">
  <p>
    I’m a dedicated <strong>Cyber Security</strong> and aspiring 
    <strong>Penetration Tester</strong>.  
    I focus on identifying vulnerabilities, securing systems, and continuously 
    learning real-world ethical hacking techniques.
  </p>
</div>

---

## 🔐 What I Do
<div align="left">
  <ul align="left">
    <li>🛡 Perform <strong>Penetration Testing</strong> on web applications</li>
    <li>🔍 Conduct <strong>Vulnerability Assessment</strong> and security analysis</li>
    <li>🌐 Practice <strong>Web Security</strong> (OWASP Top 10)</li>
    <li>⚡ Build custom <strong>Security Tools using Python</strong></li>
    <li>🧠 Learn through platforms like TryHackMe & HackTheBox</li>
  </ul>
</div>

---

## 🛠 Tech Stack (Cybersecurity)
<div align="center">
    <img src="https://skillicons.dev/icons?i=python,bash,linux,git,html,css,js,docker,react,django,mongodb,firebase,vscode&perline=6" />
</div>

---

## 📚 Technical Proficiency
<table align="center">
  <tr>
    <td><strong>Languages</strong></td>
    <td>Python, Bash, JavaScript</td>
  </tr>
  <tr>
    <td><strong>Security Tools</strong></td>
    <td>Nmap, Burp Suite, Wireshark, Metasploit</td>
  </tr>
  <tr>
    <td><strong>Focus Areas</strong></td>
    <td>Penetration Testing, Web Security, Network Security</td>
  </tr>
  <tr>
    <td><strong>Platforms</strong></td>
    <td>Linux (Kali), GitHub, Docker</td>
  </tr>
</table>

---

## 🚀 Cybersecurity Projects
<div align="left">

🔎 <a href="https://github.com/YOUR_GITHUB_USERNAME/vulerability_scanner" target="_blank">
  <strong>Vulnerability Scanner</strong>
</a>  
→ Automated tool to detect security vulnerabilities in systems

🔑 <a href="https://github.com/YOUR_GITHUB_USERNAME/password-audit-tool" target="_blank">
  <strong>Password Audit Tool</strong>
</a>  
→ Analyze password strength and identify weak credentials

🌐 <a href="https://github.com/YOUR_GITHUB_USERNAME/simple_port_scanner" target="_blank">
  <strong>Simple Port Scanner</strong>
</a>  
→ Scan open ports and detect running services on a target

</div>

---

## 🌐 Portfolio
<div align="center">
  <a href="https://portfolio.hackerbytez.com" target="_blank">
    <img src="https://img.shields.io/badge/Portfolio-View-blueviolet?style=for-the-badge" />
  </a>
  <a href="https://hackerbytez.com" target="_blank">
    <img src="https://img.shields.io/badge/HackerBytez-Visit-success?style=for-the-badge" />
  </a>
</div>

---

## 🏆 Achievements
<div align="center">
  <a href="https://tryhackme.com/p/udayraut128" target="_blank">
    <img src="https://tryhackme-badges.s3.amazonaws.com/udayraut128.png" width="250"/>
  </a>
</div>

---

## 📬 Connect With Me
<div align="center">
 <a href="https://www.linkedin.com/in/udayraut128" target="_blank"> <img src="https://raw.githubusercontent.com/maurodesouza/profile-readme-generator/master/src/assets/icons/social/linkedin/default.svg" width="52" height="40" alt="linkedin logo" /> </a>
<a href="https://twitter.com/udayraut128" target="_blank"> <img src="https://raw.githubusercontent.com/maurodesouza/profile-readme-generator/master/src/assets/icons/social/twitter/default.svg" width="52" height="40" alt="twitter logo" /> </a>
 
 
<a href="https://tryhackme.com/p/udayraut128" target="_blank">
  <img src="https://cdn.simpleicons.org/tryhackme/990000" width="40"/>
</a>
<a href="https://app.hackthebox.com/users/udayraut128" target="_blank">
  <img src="https://cdn.simpleicons.org/hackthebox/9FEF00" width="40"/>
</a>
 
 <a href="https://t.me/hackerbytez" target="_blank"> <img src="https://raw.githubusercontent.com/maurodesouza/profile-readme-generator/master/src/assets/icons/social/telegram/default.svg" width="52" height="40" alt="telegram logo" /> </a>

</div>

---

<div align="center">
  <br>
  <p>⚡ "Think like a hacker. Defend like a pro."</p>
</div>
<div align="center"> <br> <p>© 2026 Uday Raut | All Rights Reserved</p> <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=60&section=footer"> </div>
